...
class Http {
  ...
  Stream<List<SearchResponse>> makeSearchGetRequest(String searchText) async* {
    try {
      //
      http.Response response = await http.get('$googleApiUrl?key=$key&cx=$cx&q=$searchText',headers: headers);
     Map<String ,dynamic> map = json.decode(response.body);
      List<SearchResponse> result = List<SearchResponse>.from(
          json.decode(response.body)['items'].map((json) => SearchResponse.fromJson(json)));
